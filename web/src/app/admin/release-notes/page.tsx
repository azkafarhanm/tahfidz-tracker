...
import PublishReleaseNoteButton from "./PublishReleaseNoteButton";
          ...
          {releaseNotes.drafts.length > 0 ? releaseNotes.drafts.map((note) => (
            <article className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-900 dark:shadow-none" key={note.id}>
              ...
              <div className="mt-4 flex flex-wrap gap-3">
                <Link className="inline-flex min-h-10 items-center gap-2 text-sm font-semibold text-emerald-800 hover:text-emerald-950 dark:text-emerald-400 dark:hover:text-emerald-300" href={`/admin/release-notes/${note.id}/edit`}>
                  <PencilLine aria-hidden="true" size={16} strokeWidth={2.2} />
                  {t("editDraft")}
                </Link>
                <PublishReleaseNoteButton releaseNoteId={note.id} />
              </div>
            </article>
          )) : (
