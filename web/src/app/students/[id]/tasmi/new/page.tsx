...
import { createTasmiAction } from "../actions";
import { tasmiGradeOptions, tasmiStatusOptions } from "@/lib/tasmi";
...
export async function generateMetadata() {
  ...
  return { title: `${t("titleAdd")} - TahfidzFlow` };
}

export default async function NewTasmiPage({ params, searchParams }: NewTasmiPageProps) {
  const t = await getTranslations("TasmiForm");
      ...
      <section className="mx-auto flex min-h-screen w-full max-w-md flex-col px-4 py-5 sm:max-w-3xl sm:px-8">
        <header className="flex items-center justify-between gap-4">
          <div className="min-w-0">
            <Link className={backLink} href={`/students/${student.id}`}>
